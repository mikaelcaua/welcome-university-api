...
func (service *JWTService) GenerateAccessToken(user models.User) (string, error) {
	return service.generateToken(user, AccessTokenType, service.accessTokenExpirationDuration)
}

func (service *JWTService) GenerateRefreshToken(user models.User) (string, error) {
...
func (service *JWTService) generateToken(user models.User, tokenType string, expirationDuration time.Duration) (string, error) {
	now := time.Now()
