...
func NewPostgresExamRepositoryImpl(pool *pgxpool.Pool) *PostgresExamRepositoryImpl {
	return &PostgresExamRepositoryImpl{pool: pool}
}

func (repository *PostgresExamRepositoryImpl) ListApproved(ctx context.Context, filter examcontract.ApprovedExamFilter) ([]exam.Exam, error) {
...
func (repository *PostgresExamRepositoryImpl) ListPendingBySubject(ctx context.Context, subjectID int64) ([]exam.Exam, error) {
	return repository.list(ctx, baseExamQuery()+` WHERE e.status = 'PENDING' AND e.subject_id = $1 ORDER BY e.id ASC`, subjectID)
}

func (repository *PostgresExamRepositoryImpl) ListPendingByUploader(ctx context.Context, userID int64) ([]exam.Exam, error) {
	return repository.list(ctx, baseExamQuery()+` WHERE e.status = 'PENDING' AND e.uploaded_by_id = $1 ORDER BY e.id ASC`, userID)
}

func (repository *PostgresExamRepositoryImpl) CountPendingByUploader(ctx context.Context, userID int64) (int64, error) {
	var total int64
	err := repository.pool.QueryRow(ctx, `SELECT count(*) FROM exam WHERE uploaded_by_id = $1 AND status = 'PENDING'`, userID).Scan(&total)
	return total, err
}

func (repository *PostgresExamRepositoryImpl) FileHashExists(ctx context.Context, fileHash string) (bool, error) {
	var exists bool
	err := repository.pool.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM exam WHERE file_hash = $1)`, fileHash).Scan(&exists)
	return exists, err
}

func (repository *PostgresExamRepositoryImpl) Create(ctx context.Context, exam exam.Exam) (exam.Exam, error) {
	var subjectID any
...
func (repository *PostgresExamRepositoryImpl) FindByID(ctx context.Context, examID int64) (exam.Exam, bool, error) {
	exams, err := repository.list(ctx, baseExamQuery()+` WHERE e.id = $1`, examID)
	if err != nil {
		return exam.Exam{}, false, err
	}
	if len(exams) == 0 {
		return exam.Exam{}, false, nil
	}
	return exams[0], true, nil
}

func (repository *PostgresExamRepositoryImpl) UpdateReview(ctx context.Context, exam exam.Exam) (exam.Exam, error) {
	var reviewerID any
	if exam.ReviewedBy != nil {
...
func (repository *PostgresExamRepositoryImpl) list(ctx context.Context, query string, args ...any) ([]exam.Exam, error) {
