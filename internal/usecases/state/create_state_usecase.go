...
type CreateStateUseCase struct {
...
func (useCase *CreateStateUseCase) Execute(ctx context.Context, code string, name string) (state.State, error) {
	createdState, valid := state.NewState(code, name)
	if !valid {
		return state.State{}, domainerrors.Validation("Sigla e nome sao obrigatorios.")
	}
	_, found, err := useCase.states.FindByCode(ctx, createdState.Code)
	if err != nil {
		return state.State{}, err
	}
	if found {
		return state.State{}, domainerrors.Conflict("Estado ja cadastrado: " + createdState.Code)
	}
	return useCase.states.Create(ctx, createdState)
}
