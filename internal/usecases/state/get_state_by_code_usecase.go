...
type GetStateByCodeUseCase struct{ states statecontract.StateRepositoryContract }
...
func (useCase *GetStateByCodeUseCase) Execute(ctx context.Context, code string) (state.State, error) {
	state, found, err := useCase.states.FindByCode(ctx, strings.ToUpper(strings.TrimSpace(code)))
	if err != nil {
